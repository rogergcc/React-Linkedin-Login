...
const { parseRsstoJson } = require('rss-to-json');
...
router.get('/indeed', async function(req, res, next) {
  console.log("INGRESO METHODO INDEED")
  requestJobsIndeed("developer")
  .then((response) => {
    console.log(response)
    //res.render('indeedCallback', { profile: response.body});
  })
  .catch((error) => {
    res.status(500).send(`${error}`)
    console.error(error)
...
async function requestJobsIndeed(jobtoSearch) {
  // var rss = await parse('https://blog.ethereum.org/feed.xml');
  var rss = await parseRsstoJson('https://pe.indeed.com/rss?q=developer&l=Per%C3%BA');
